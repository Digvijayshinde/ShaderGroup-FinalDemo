...
struct imageData
{
    ...
    imageMipData mip[MAX_TEXTURE_MIPS];
};
...
static bool convertDDSHeaderToImageDataHeader(const DDS_FILE_HEADER &header, imageData* image)
{
...
static GLsizei getDDSStride(const DDS_FILE_HEADER& header, GLsizei width)
{
    if (header.std_header.ddspf.dwFlags == DDS_DDPF_FOURCC && header.std_header.ddspf.dwFourCC == DDS_FOURCC_DX10)
    {
        if (header.dxt10_header.format < NUM_DDS_FORMATS)
...
static GLenum getTargetFromDDSHeader(const DDS_FILE_HEADER& header)
{
...
void loadDDSFile(const char* filename, imageData* image)
{
    FILE* f;
    ...
    if (!convertDDSHeaderToImageDataHeader(file_header, image))
        goto done_close_file;
    ...
    for (level = 0; level < image->mipLevels; ++level)
    {
        ...
        image->mip[level].mipStride = getDDSStride(file_header, width) * height;
...
GLuint loadDDSTexture(const char* filename, GLuint texture, imageData* image)
{
    ...
    loadDDSFile(filename, image);

    unsigned int size = image->mip->width * image->mip->height;
...
GLuint loadDDSTexture_Custom(const char* filename, imageData* image, bool clampTexture=false)
{
    imageData localImage;
    int level;
    GLuint texture=0;
    ...
    glTexParameteri(image->target, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    ...
    glTexParameterf(image->target, GL_TEXTURE_LOD_BIAS, 0.0f);
    glTexParameterf(image->target, GL_TEXTURE_MAX_ANISOTROPY_EXT, 1.0f);
    glTexParameteri(image->target, GL_TEXTURE_COMPARE_MODE, GL_ALWAYS);
    glTexParameterf(image->target, GL_TEXTURE_MIN_LOD, 0.0f);
    glTexParameterf(image->target, GL_TEXTURE_MAX_LOD, FLT_MAX);

    glTexImage2D(image->target, 0, GL_RGBA, image->mip->width, image->mip->height, 0, GL_BGRA, GL_UNSIGNED_BYTE, image->mip->data);
