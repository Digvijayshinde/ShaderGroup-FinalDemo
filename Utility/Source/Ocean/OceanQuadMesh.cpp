...
void OceanQuadMesh::Initialize(const Math::Vector2& start, float size, int lodcount, int meshsize, float patchsize, float maxgridcoverage, float screensize)
{
	root.start	= start;
	root.length	= size;
...
void OceanQuadMesh::Rebuild(const Math::Matrix& viewproj, const Math::Matrix& proj, const Math::Vector3& eye)
{
	nodes.clear();
...
int OceanQuadMesh::BuildTree(Node& node, const Math::Matrix& viewproj, const Math::Matrix& proj, const Math::Vector3& eye)
{
...
bool OceanQuadMesh::IsVisible(const Node& node, const Math::Matrix& viewproj) const
{
	Math::AABox box;
...
int OceanQuadMesh::FindLeaf(const Math::Vector2& point) const
{
	int index = -1;
...
void OceanQuadMesh::FindSubsetPattern(int outindices[4], const Node& node)
{
	outindices[0] = 0;
	outindices[1] = 0;
	outindices[2] = 0;
...
void OceanQuadMesh::Traverse(NodeCallback callback) const
{
	if (nodes.size() == 0)
		return;
...
float OceanQuadMesh::CalculateCoverage(const Node& node, const Math::Matrix& proj, const Math::Vector3& eye) const
{
...
void OceanQuadMesh::InternalTraverse(const Node& node, NodeCallback callback) const
{
	if (node.IsLeaf()) {
		callback(node);
	} else {
