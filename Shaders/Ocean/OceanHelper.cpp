...
bool createTextureFromDDSFile(const char* file, bool srgb, GLuint& texture)
{
	//code
	DDS_Image_Info info;

	if (!LoadFromDDS(file, &info)) {
		PrintLog("Error: Could not load texture!\n");
		return false;
	}
	...
	if (err != GL_NO_ERROR)
	{
		...
		PrintLog("Error: Could not create texture!\n");
		return (false);
	}
	else 
	{
		PrintLog("Created texture %u %u\n", info.Width, info.Height);
		return (true);
	}
}
